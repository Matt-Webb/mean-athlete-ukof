...
function seedCity() {
    ...
    if(City.count(function(err, count){
        if(!err && count === 0) {
            var cities = require('../../data/locations.json');
            if(cities) {
                City.collection.insert(cities, onInsert);
                console.log(chalk.green('New Cities added'), cities.length);
            } else {
                console.log(chalk.red('There was an error retreiving the cities data'));
            }
        } else if (err) {
            console.log(chalk.red('Cities error'), err);
        } else {
            console.log(chalk.yellow('Cities have already been added'), count);
        }
    }));
}
    ...
    if(Article.count(function(err, count) {
        if(!err && count === 0) {
            ...
            if(articles) {
                ...
                console.log(chalk.green('New Articles added'), articles.length);
            } else {
    ...
    if(Instructor.count(function(err, count) {
        if(!err && count === 0) {
            ...
            if(instructors) {
                ...
                console.log(chalk.green('New Instructors added'), instructors.length);
            } else {
...
function seedVenues() {
    ...
    if(Address.count(function(err, count) {
        if(!err && count === 0) {
            ...
            if(addresses) {
                ...
                console.log(chalk.green('New Addresses added'), addresses.count);
            } else {
    ...
    if(Venue.count(function(err, count) {
        if(!err && count === 0) {
            var venues = require('../../data/venues.json');

            if(venues) {
                Venue.collection.insert(venues, onInsert);
                console.log(chalk.green('New Venues added'), venues.length);
            } else {
                console.log(chalk.red('There was an error retreiving the venue data'));
            }

        } else if(err) {
            console.log(chalk.red('Venue details error'), err);
        } else {
            console.log(chalk.yellow('Venue details have already been added!'), count);
        }
    }));
}
